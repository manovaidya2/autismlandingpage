...
export default function AutismBookingModal({ open, setOpen }) {
  ...
  const validateForm = () => {
    ...
    if (!formData.date) {
      newErrors.date =
        "Date is required (Tuesday, Thursday, or Saturday only)";
    } else {
      const selectedDate = new Date(formData.date);

      if (!isDateAllowed(selectedDate)) {
        newErrors.date =
          "Please select Tuesday, Thursday, or Saturday";
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        notes: `
Service: Autism Neuro Assessment
Date: ${formData.date}
...
Mode: ${formData.mode}
      ...
      localStorage.setItem(
        "autismBookingFormSubmitted",
        "true"
      );

      localStorage.setItem(
        "autismBookingFormData",
      ...
      console.error("Autism Kraya Lead Error:", err);
      setSubmitError(
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 p-4">
      <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-[28px] bg-white shadow-2xl">
        ...
        <div className="px-5 pb-7 sm:px-7">
          ...
          <p className="mt-2 text-[14px] text-[#6b756c]">
            Fill the details below. Available on Tuesdays,
          ...
          <form onSubmit={handleSubmit} className="mt-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="relative calendar-container">
                ...
                {showCalendar && (
                  <div
                    className="absolute z-20 mt-2 w-full bg-white rounded-2xl border border-[#e5ddcf] shadow-xl p-4"
                  ...
                  >
                    ...
                    <div className="grid grid-cols-7 gap-1 mb-2">
                      {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map(
                        (day) => (
                          <div
                            key={day}
                            className="text-center text-xs font-medium text-[#6b756c] py-2"
                          >
                            {day}
                          </div>
                        )
                          ...
                          <button
                            ...
                            className={`
                              ...
                              ${
                                allowed &&
                                !selected &&
                                "hover:bg-[#d6a22e]/20 cursor-pointer"
                              }
                              ${
                                selected &&
                                "bg-[#d6a22e] text-white font-semibold"
                              }
                              ${
                                today &&
                                !selected &&
                                allowed &&
                                "border border-[#d6a22e] bg-[#d6a22e]/5"
                              }
                            `}
                          >
                            {date.getDate()}
                    ...
                    <div className="mt-4 pt-3 border-t border-[#e5ddcf] flex items-center justify-center gap-4 text-xs">
                      <div className="flex items-center gap-1">
                        <div className="w-3 h-3 rounded-full bg-[#d6a22e]" />
                        <span className="text-[#6b756c]">Available</span>
                      </div>
